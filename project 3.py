...
def add_friendship(network, name1, name2):
    ...
    if network[name1].count(name2)==0 and network[name2].count(name1)==0:
...
def print_command(network, namelist):
    ...
    for val in sorted(network.keys()):
        print(val)
        for val2 in sorted(network[val]):
            print(f"- {val2}")
...
def friends_command(network, namelist):
    ...
    for val in sorted(network[name]):
        print(val)
...
def common_friends_command(network, namelist):
    ...
    for val1 in network[name1]:
        for val2 in network[name2]:
            if val1==val2:
                commonFriends.append(val1)

    if len(commonFriends)>0:
        for val in sorted(commonFriends):
            print(val)
    else:
        print(f"{name1} and {name2} have no common friends.")
